	...
	if (event.type == SDL_KEYDOWN) {
		if (event.key.keysym.sym == SDLK_LEFT) { // Camera left
			...
			if (mode == State::RASTERISED) {
				...
				window.clearPixels();
				MyModel myModel;
				cameraPosition = cameraPosition + leftMatrixRay;
				myModel.drawRasterisedSceneOrbit(window);
			}
			...
			if (mode == State::RASTERISED) {
				...
				window.clearPixels();
				MyModel myModel;
				cameraPosition = cameraPosition + rightMatrixRay;
				myModel.drawRasterisedSceneOrbit(window);
			}
			...
			if (mode == State::RASTERISED) {
				...
				window.clearPixels();
				MyModel myModel;
				cameraPosition = cameraPosition + upMatrixRay;
				myModel.drawRasterisedSceneOrbit(window);
			}
			...
			if (mode == State::RASTERISED) {
				...
				window.clearPixels();
				MyModel myModel;
				cameraPosition = cameraPosition + downMatrixRay;
				myModel.drawRasterisedSceneOrbit(window);
			}
			...
			*/
			...
			orbMode = Orb::OFF;
		} else if (event.key.keysym.sym == SDLK_y) { // y: Reset model to raytrace
			std::cout << "y" << "/" << "Y" << ":" << "Reset to raytrace" << std::endl;
			window.clearPixels();
			MyModel myModel;
			myModel.resetDepthBuffer();
			myModel.resetModel();
			...
			if (mode == State::RASTERISED) {
				...
				window.clearPixels();
				MyModel myModel;
				cameraPosition = cameraPosition * rotationMatrixYRay; // Left
				myModel.drawRasterisedSceneOrbit(window);
			}
			...
			if (mode == State::RASTERISED) {
				...
				window.clearPixels();
				MyModel myModel;
				cameraPosition = cameraPosition * rotationMatrixYYRay; // Right
				myModel.drawRasterisedSceneOrbit(window);
			}
			...
			if (mode == State::RASTERISED) {
				...
				window.clearPixels();
				MyModel myModel;
				cameraPosition = cameraPosition * rotationMatrixXRay; // Up
				myModel.drawRasterisedSceneOrbit(window);
			}
			...
			if (mode == State::RASTERISED) {
				...
				window.clearPixels();
				MyModel myModel;
				cameraPosition = cameraPosition * rotationMatrixXXRay; // Down
				myModel.drawRasterisedSceneOrbit(window);
			}
...
int main(int argc, char* argv[]) {
	...
	// Raytrace render
	// myModel.drawRasterisedScene(window); // Rasterised render with ray trace
	// mode = State::RAYTRACING;

	// Raytrace render with orbit & lookAt
	myModel.drawRasterisedSceneOrbit(window); // Ray trace render with orbit
	mode = State::RAYTRACING;
	...
	while (true) {
		...
		if (orbMode == Orb::ON) {
			if (mode == State::RASTERISED) {
				...
				MyModel myModel;
				/* -------------------- */
				cameraPosition = cameraPosition * rotationMatrixYYRay; // Move camera towards left of the box
				cameraOrientation = myModel.lookAt(cameraPosition, targetPoint, upVector); // Let camera focus on the centre
				/* -------------------- */
				myModel.drawRasterisedSceneOrbit(window);
			}
